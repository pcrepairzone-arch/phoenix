...
/* Block read */
ssize_t mmc_block_read(blockdev_t *bdev, uint64_t lba, uint32_t count, void *buf)
{
    mmc_ctrl_t *ctrl = bdev->private;

    writel(SDHCI_BLOCK_SIZE, ctrl->regs + SDHCI_BLOCK_SIZE);
    writel(count, ctrl->regs + SDHCI_BLOCK_COUNT);

    mmc_send_cmd(CMD_READ_SINGLE_BLOCK, lba, 0x10);  // Data enable

    // Read data from buffer
    for (int i = 0; i < count * 512 / 4; i++) {
        ((uint32_t*)buf)[i] = readl(ctrl->regs + SDHCI_BUFFER);
    }
...
/* Block write */
ssize_t mmc_block_write(blockdev_t *bdev, uint64_t lba, uint32_t count, const void *buf)
{
    mmc_ctrl_t *ctrl = bdev->private;
    ...
    for (int i = 0; i < count * 512 / 4; i++) {
        writel(((uint32_t*)buf)[i], ctrl->regs + SDHCI_BUFFER);
    }

    mmc_send_cmd(CMD_WRITE_SINGLE_BLOCK, lba, 0x10);  // Data enable

    return count * 512;
}

/* IRQ handler stub */
static void mmc_irq_handler(int vector, void *private) {
...
{
    if (mmc_init_controller() == 0) {
        irq_set_handler(MMC_IRQ_VECTOR, mmc_irq_handler, NULL);
        irq_unmask(MMC_IRQ_VECTOR);
    }
    debug_print("MMC driver loaded\n");
    return NULL;
}